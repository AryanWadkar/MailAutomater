    ...
    try{
        const response = await gmail.users.messages.list(
            ...
            const processEmail = async (messageId,threadId) => {
                ...
                const replied = await hasRepliedToEmail(gmail, messageId);

                if (!replied) {
                    const replyMessage = 'This mail is being auto replied to by MailAutomater';
            ...
            processEmail(messages[0].id,messages[0].threadId);
    }catch(err){
        throw(err);
    }
}
...
async function replyToEmail(gmail, messageId, replyMessage) {
    try{
          ...
          replyData.raw = `To: ${originalMessage.payload.headers.find((header) => header.name === 'From').value}\r\n`; // Reply to the original sender
          replyData.raw += `Subject: Re: ${originalMessage.payload.headers.find((header) => header.name === 'Subject').value}\r\n`; // Include "Re:" in the subject
